  ...
  return (
    <section className={hasSide ? "wiki-pane" : "wiki-pane wiki-pane--no-side"} aria-label="Wiki">
      <div className="pane-bar">
        <span className="pane-title">Wiki</span>
        <Pill>
          {index.pages.length} {index.pages.length === 1 ? "page" : "pages"}
        </Pill>
        {/* Where the open page actually sits. The old page bar said this and
            it is worth keeping: a folder is not an address, but "which file am
            I reading" is still a fair question. */}
        {page ? <code className="pane-bar__path">{page.path}</code> : null}
