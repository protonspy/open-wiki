...
- [ ] 6.3 (Unit) A transcribe button on a stopped recording, with per-chunk progress and the option to redo only what failed
  - **Not done, and the box came back.** The per-chunk progress is on the sources screen, because `sourceState` derives it from the journal for free — but there is no transcribe button, no retry-what-failed, and nothing that starts a run. It needs the credential of 8.3, which is where it belongs: the orchestrator that reads the Groq key is the desktop application, and 4.15 deliberately left that wiring here rather than in the CLI.
