...
- An inverted index, embeddings or a vector store. Full-text search over the files, yes.
...
<workspace>/
  fenix/                        one project
    raw/                        sources, immutable once written
      2026-07-31T14-02-11Z/       a recording
        manifest.json · mic.opus · system.opus · timeline.json · text.md
      arquitetura-fenix.pdf/      an uploaded file
        source.pdf · text.md
...
- [ ] 3.1 (Unit) Register a source in `raw/<id>/` with the original file preserved, marking it immutable once written
...
- [ ] 4.4 (Unit) Emit `manifest.json` with the absolute timestamp of each track's first frame and the pause intervals
...
- [ ] 4.9 (Unit) Transcribe chunks in parallel, isolating failure and redoing only the chunk that failed
...
the recording; never deleting fills the disk in twenty meetings.
