...
- [x] 7.1 (Unit) The sources pane shows each source as what it is: title, description, kind, size, status, and what cites it — and offers the two actions that are not the agent's, correcting a title and marking something processed by hand. A row whose only readable field is `fnd348r34nr483r.txt` is the row this whole group exists for
  - **Size is the file that arrived, not the directory around it.** Totalling the directory means walking it, and an unpacked archive (6.1) is thousands of files — a listing of twenty sources would stat a hundred thousand of them to render one column. It is also the more honest number: the archive is what arrived and what a citation can be checked against, and the tree beside it is a convenience `ow` can produce again, which is why 6.5 keeps it out of git.
  - **Marking read by hand is the second of the two actions that are not the agent's**, and it goes through the same single manifest mutator `ow source mark` uses (`specs/source-status` R2.1) rather than growing a second writer in the Electron process — which is the rule that was written after finding exactly that here.
  - The description finally reaches the screen, which is the whole point of the row: `fnd348r34nr483r.txt` was a row whose only readable field said nothing, and 8.1 wrote the answer that nobody was showing.
  - **It also closes the gap group 6's review left open**: `unpacked` and `unpackFailed` were dropped on the floor, so an archive that hit the bomb bound read as "it was added" with no sign the tree was missing. `incomplete` and the size column say what the source actually is.
  - **The desktop's mark diverged from the CLI's, and only its one caller hid it.** `runSourceMark` is idempotent and keeps the original date; this wrote today's unconditionally. Nothing reached it twice — the button offers "Mark read" only when there is no declaration — so it was correct by accident rather than by contract. A code review caught it. Two doors onto one act must not disagree about what the act is.
- [x] 7.2 (Unit) 6.6's uncited check reports only what is unprocessed *and* uncited, so a source somebody read and discarded stops being a permanent finding
  - **Done by `specs/source-status/`** (R4.1, R4.2), which needed the two-fact rule to settle what the declaration meant and could not leave the check reading one fact without contradicting itself. `checkRecords` skips a source that is cited *or* declared, and the tests for both live in `packages/access/tests/check.spec.ts`.
  - Ticked here rather than re-implemented: a second reading of the same rule is the duplicate the plan spends 9.1 avoiding.
- [x] 7.3 (Unit) MCP says what it has: a source with no `text.md` reports its status and its filename rather than returning nothing, so a consulting agent knows the difference between empty and unread. It cannot read the original — that project is not on its disk — and saying so is the honest answer
  - **One shape with a discriminator, not a string that is sometimes an apology.** A consulting agent has to branch on *whether there is text* without pattern-matching prose, so the answer is `{hasText: true, text}` or `{hasText: false, …what it is}`.
  - Since `adr:0021` most sources have no `text.md` at all — a PDF, an image, an archive — and throwing `ENOENT` told the caller only that something went wrong. It could not tell **empty** from **unread**, which are different answers with different next steps.
  - **It says the original is out of reach rather than pretending otherwise.** That project is not on the consulting agent's disk and MCP serves it over HTTP (`adr:0018`), so naming the file is as far as honesty goes — and is what makes it actionable for somebody who can reach the machine.
  - A supersession travels in the same answer, so a consulting project cannot read replaced evidence without being told it was replaced.
- [x] 7.4 (Unit) The provenance viewer opens what it is given: an image as an image, a PDF at its page, an unpacked tree as a file listing, anything else named and offered to the system handler. The renderer's CSP is `default-src 'none'` and `img-src 'self' data:`, so this is a real constraint and not a formality
  - **The CSP is a real constraint and the type system now carries it.** `image` is its own location kind because `img-src 'self' data:` means those bytes can reach an `<img>` and a PDF's cannot; collapsing the two would push that decision into the renderer at the moment it is least able to answer it.
  - **`.svg` is deliberately not an image here.** It is a document that can carry script and it arrived from a source nobody parsed, so it goes to the system handler like any other file rather than into a renderer that trusts `img-src`.
  - An unpacked archive answers with its tree *before* the "no file" branch: there is a file — the zip — and opening it in a viewer shows nobody anything.
  - `external` is what keeps `adr:0021`'s "any file" from quietly meaning "any file we listed": anything not an image, a PDF or a tree is named and offered to the system.
  - **`img-src` has no `file:` in it, and that turned out to be the whole design.** `media-src 'self' file:` is how the audio player loads an Opus off disk; the image branch cannot do the same, so the bytes travel as a `data:` URL, which is what the CSP does allow. Widening `img-src` would have answered a constraint the task calls real by removing it. An image past 8 MiB degrades to `external` — a panel is not where a screenshot that size is read anyway.
  - **`external` reveals rather than opens**, and the choice is deliberate. `shell.openPath` invokes whatever handler an extension is registered to, on bytes that arrived in an untrusted clone — a `.lnk`, a `.scr`, a double extension. This repository already treats that as load-bearing: `isOpenableExternally` exists because `shell.openExternal` is `ShellExecute` on Windows. `showItemInFolder` is the offer the task asks for; opening is one further click, taken by the person, where their own operating system's warnings apply.
  - The viewer itself was missing until a review said so: `locateCitation` returned three new kinds and `SourceAt.tsx` branched on none of them, so the behaviour the task describes did not exist in the shipped app. Ticking a backend as a user-facing task is the failure mode worth naming.
  - **The directory being confined was not enough, and a security review found where.** `resolvedSourceDir` resolves and confines the source directory, so a junction standing where it should be is refused — but the file *inside* it was simply joined and handed to `readFileSync` and to `shell.showItemInFolder`. A repository shipping `raw/leak.png/source.png` as a symlink to a key therefore read that file's bytes into a `data:` URL and across IPC into the renderer, on one click of what looks like an ordinary citation. `fileIn` is the one lookup now: `lstatSync` sees the link rather than its target, `isWithin` catches whatever the link would have reached, and the audio branch goes through it too. It was the last path in this module doing neither, while `browseSource` skipped symlinked entries and `safeRoot` resolved its root.
  - The three regression cases are **skipped, not passed**, where a file symlink needs privilege — Windows without Developer Mode. A security test that quietly returns early reads as coverage it does not have, so the case that needs no privilege (a directory standing where the original should be) is asserted unconditionally beside them.
- [x] 7.5 (Unit) Browse into a source: the files it holds, and an unpacked tree as a tree. Reading one is the agent's job, but *seeing what arrived* is how somebody knows the upload was what they meant
  - **It returns names and sizes and opens nothing.** Reading a source is the agent's job; seeing what arrived is how somebody knows the upload was what they meant — that the zip really was the repository and not last month's.
  - Where a source holds an unpacked tree, that tree is what is browsed rather than the two files beside it: `source.zip` and `contents/` is a listing nobody came to see.
  - **Bounded at 2000 entries, and it says how many it did not show.** An unpacked repository is thousands of files crossing an IPC channel into a renderer that will lay every one of them out — the same rendering argument as `boundedText`, and it destroys nothing.
  - Links are not followed, for the reason `listSourceRefs` does not follow them: one pointing out of `raw/` would list somebody else's disk as this source's contents.
  - **This is where 6.6's marker finally gets a reader.** A security review pointed out it had none — a crash mid-unpack left a partial tree that every reader walked into as though it were whole. `sourceState`, the browser and the viewer all say so now.
  - **`contents/` can itself be a junction, and walking it listed an arbitrary directory.** The recursion skipped a symlinked *entry*, which is the easier half; the root it started from was never resolved. `export/zip.ts` had to add the identical "the tree root is checked too" fix after the same finding. A code review reproduced it here.
  - **`truncated` undercounted by two orders of magnitude.** Skipping an over-cap directory outright meant none of its descendants were counted, so a tree of 2000 files plus a directory of 500 reported `1`. Counting continues past the cap now; only pushing stops. A number whose only job is to be right about what was not shown was wrong.
  - As with 7.4, no renderer called it until a review pointed that out. There is a **Files** toggle on each row now, and the citation panel renders an unpacked tree.
  - **And making `truncated` exact left the walk unbounded**, which a second review round caught. Nothing upstream closes that: `unpackArchive` caps total bytes and the expansion ratio, and an archive of a million empty files or a thousand nested directories passes both — then every browse of that source walks the whole tree synchronously in the Electron main process, and the recursion is an uncaught `RangeError` rather than a slow answer. There are two bounds on the walk now, separate from the listing cap because they answer a different question: how much work the process will do to say what it did not show. Past them `atLeast` says the number is a floor, because "1,438 more" and "at least 38,000 more" are different answers.
- [x] 7.6 (Unit) A superseded source says so wherever it is shown, and points at what replaced it — the same thing 8.5 and 6.5 already do for a page and for a broken citation. A citation into a replaced source resolving silently is the outcome supersession exists to prevent
  - Said on the **page's own list of what it rests on**, not only on the sources screen somebody may never open. A citation into replaced evidence resolving silently is the outcome supersession exists to prevent, and the page is where a reader meets the citation.
  - It reaches the row and the MCP answer from the same `SourceState` field 8.5 put there, so the three cannot drift.
