...
- [ ] 5.6 (Unit) The three fix buttons the draft draws that a finding cannot yet be acted on with. Each needs the check to carry what it already has in hand at the moment it reports: the target of a broken wikilink, the last instant a recording actually contains, and the synonym beside the canonical term. That is a change to `Finding` — which `ow check --json`, MCP and a CI job all read, so it is an interface — and *Replace* needs a write that does not exist, rewriting a word inside a page through the gate. The plan's own scope says this group does not touch `@open-wiki/access`, and this is the task that has to
