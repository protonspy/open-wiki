...
- [x] 5.3 (Unit) The fix buttons that a finding can actually be acted on with: *Add to index*, *Open the source*, and *Open the page* for anything that names one. Reaching an existing operation from the finding that named it, which was this task's premise
  - **The premise held for two of the draft's five.** A `Finding` carries a code, a severity, a message, a fix, and — where the check could point at one — a page, a source and a line. It does **not** carry the slug a broken wikilink names, the instant a citation overran, or the pair of words a synonym finding is about: those exist only inside the prose of `message`. So *Create the page*, *Open at 58:04* and *Replace* are not built, and 5.6 below is what they need. Cutting the value back out of the sentence is not an option — `checks.ts` names that failure at the very site that would produce it, having already been bitten by it once.
  - *Add to index* is `registerInIndex`, which `createPage` already calls as part of creating a page; on its own it is `addToIndex`, recorded like any other write so it undoes. Nothing is recorded when the page was already linked, because an operation offering to undo a write that never happened is exactly what 8.11 promises the history is not.
  - A fix is offered only when it can be taken: some findings are about `wiki/changelog.md`, which is not an entity page and is not in the index, so *Open the page* would have failed from a button the pane itself offered.
- [x] 5.4 (TDD) The provenance viewer: transport, the instant, seek-to-citation, copy this citation. Test-first because it is the time map's last mile — 8.6 seeks to an instant, and an off-by-one here points a citation at the wrong moment while reading perfectly, which is the failure family the plan's own notes reserve TDD for
  - **Red observed** first: 9 assertion failures across `citationOf`, `playheadPercent` and `transportLabel` against signature-only stubs. Two of the eleven passed against the stubs — the `null` and the `0` cases, which a stub returning nothing satisfies by accident — and that is worth saying rather than claiming eleven reds.
  - **Nothing here defines a time format.** `formatInstant` in `@open-wiki/audio/timemap` is the one authority, the same one `store/provenance.ts` validates against and 4.13's headings are written with. The round-trip is asserted through `parseInstant` itself: what this pane copies has to parse back to the instant it was copied at, or the reader pastes a citation the checks refuse.
  - **Copy gives the citation for where the transport is now**, not the fragment the panel was opened with. Listening on to find the moment a claim was actually made is the point; a copy button handing back the citation you already had would make that the one thing the panel cannot do.
  - The playhead is clamped at both ends. A citation past the end is refused upstream, but 4.6 tolerates a map that disagrees with its file by up to a second — and a playhead at 103% is drawn outside the element it belongs to.
- [x] 5.5 (Unit) The waveform, drawn from the Opus. Static and rendered once per source, not live — the draft is explicit that a waveform earns its keep here and nowhere else
  - **It costs more than the draft's canvas suggests, and the reason is the CSP.** The renderer is `default-src 'none'`, so it cannot `fetch` the Opus to decode it: `media-src 'self' file:` lets the `<audio>` element play the file and nothing lets script read its samples. So the peaks have to be computed in the main process — which has ffmpeg bundled, so `ffmpeg -i mic.opus -f s16le -` plus a reduction is the shape — and reach the renderer over a channel of their own, cached per source because an hour of audio is not decoded twice.
  - **Built that way**: ffmpeg decodes to 1 kHz mono PCM in a temporary file, `peaksFrom` buckets it into a column per pixel of the panel, and the renderer receives a few hundred numbers. What a resample yields is an *envelope* rather than the true sample peaks — which is what a waveform is for: the loud parts survive, and where they are is the whole question somebody has.
  - **The bucket boundaries come from the column, never from a stride.** A stride accumulates rounding and the last column stops short of the end, which is exactly the misalignment between the picture and the playhead this panel exists to avoid.
  - **An SVG, not the draft's canvas.** A canvas needs a pixel width to draw into and the panel's is whatever the window is, so it would want a resize observer and a device-pixel-ratio multiplier to say what an SVG scales for free. The draft drew a canvas because it was a static mock at a fixed width; nothing else about it argued for one.
  - Decoded once per source and held in memory: sound in `raw/` is sealed (`adr:0011`, 4.14), so a cached answer cannot go stale. Not on disk — surviving a restart is worth little, and a cache file under `.state/` would be a new write path in the directory undo owns.
  - A file source is never asked about, and a recording with no Opus yet answers null rather than an error: the panel's subject is the citation, and it draws the plain position bar instead. **That ffmpeg accepts these arguments is unverified**, like every other ffmpeg invocation in this repository — CI has no ffmpeg, so it joins group 4's standing manual checks.
- [x] 5.6 (Unit) The three fix buttons the draft draws that a finding cannot yet be acted on with — *Create the page*, *Open at 58:04*, *Replace*. Each needs the check to carry what it already holds at the moment it reports: the target of a broken wikilink, the last instant the recording actually contains, and the synonym beside the canonical term. That is a change to `Finding`, which `ow check --json`, MCP and a CI job all read — an interface, so a decision and not a field — and *Replace* additionally needs a write that does not exist, rewriting a word inside a page through the gate. This group's own scope says it does not touch `@open-wiki/access`; this is the task that has to
  - **`adr:0023` — a finding carries what it found.** `Finding` is read by `ow check --json`, by MCP and by a CI job, so this is an interface and the change is a record rather than three fields somebody added. `target`, `endsAt` and `replace` are optional in the shape `page`, `source` and `line` already established: present where the check could point at one, absent where it could not.
  - **Nothing parses `message`, and nothing here ever will.** 5.3 refused to build these buttons for exactly that reason, and `checks.ts` had already been bitten once — cutting the target back out of the prose *produced garbage the moment the wording changed*. The general rule the ADR states, for the checks not yet written: a check that formats a value into its message puts that value on the finding.
  - **`provenance.unresolved` gained `source` as well as `endsAt`.** The finding named the page and not the recording, so *Open at 58:04* had nothing to open — the button would never have fired, which is the quiet kind of not-built.
  - **The write shares one definition of prose with the check that reports it** (`store/prose.ts`). A rewrite that stripped a different set of literals would change a word the check never complained about — a variable inside a fence, the target of a link that resolves — and leave the finding standing after the button said it had fixed it. The test asserts the two *agree*, by running `checkProject` before and after, rather than asserting each alone.
  - It lives below both, importing nothing, because `checks.ts` is part of the read surface the MCP process loads (plan 9.9) and the rewrite calls the gate. Putting the rule beside the rewrite pulled write code into a surface whose whole guarantee is that it has none — the same trap `CONTENTS` and `UNPACKING` were moved out of `archive.ts` to avoid, hit again the same week.
  - **Frontmatter is never rewritten**: `title`, `id` and `aliases` are the page's identity and the glossary's own definitions, and a synonym sweep that renamed a title would rename the concept it was trying to spell consistently. The test asserts the block rather than its formatting, because the gate re-serialises what it accepts.
  - **The rewrite was confined to the project and not to `wiki/`**, which a security review turned into rewriting `package.json`. `gateWrite` refuses `.claude/`, `.mcp.json` and `CLAUDE.md` and *allows* everything else outside `wiki/`, so the confinement that mattered was the one this did not do — and `socket.ts` already draws that exact distinction for its read verb. The legitimate caller only ever passes `finding.page`; the IPC channel enforces none of that, and whatever runs in the renderer can call it with three strings of its choosing.
  - **And the fields `adr:0023` added bypassed `safe()`.** `message` and `fix` have been scrubbed and bounded since group 7 for a stated reason — a cursor escape in an alias can erase the summary a human reads — and the new fields were assigned raw two lines below them. The review carried a two-megabyte target and a control sequence straight into `ow check --json`. They go through `safe()` now, which is what every other interpolated value on the same object already did.
  - **And *Create the page* was offered where it could not be taken**, which a code review caught: every other kind in `fixesFor` gates on `knownSlugs`, and the file's own comment says why — *a fix nobody can take is worse than none, because the reader spends a click finding that out*. Two ways it could not: the page was written since the check ran, and a target that is not a name at all, which `assertSlug` refuses before a path is built from it. The slug rule moved to `shared/pages.ts` so both sides of the process boundary read one definition — the renderer may not import `main/edit.js`, whose value graph reaches `node:fs`.
...
- [x] 6.3 (Unit) First run as the draft's four steps — project, language, transcription, done — with step 1 picking a project directory rather than a workspace, per the table above. The language step already exists in `Launcher.tsx` and moves here rather than being rewritten
