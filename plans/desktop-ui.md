...
- [ ] 8.4 (Unit) The recording indicator earns the draft's persistence: elapsed time, and the pause/stop controls in the titlebar where 8.2 argued they belong — visible from every pane, which is the reason the plan gave for putting them there
- [ ] 8.5 (Unit) The status bar: the project path, the findings count as a way into Checks, and Undo last write
