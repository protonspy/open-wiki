...
- **smol-toml** — parses Codex's `.codex/config.toml` so `ow consult add` can tell whether its entry is already there (`plans/harness-portability.md` 4.1). **Parsing only; the write is an append.** Round-tripping a config through a serialiser returns it with every comment and ordering choice gone — deleting somebody's `# why we set this` in order to add four lines to a file we do not own — so the parse answers one question (is this valid TOML, and is our table already in it) and the entry is appended as text. A real parser rather than a regex, because the alternative is a wrong answer about a file Codex then refuses to load, which is the failure this whole plan is about; and because a second `[mcp_servers.x]` is not untidy but *invalid*, so knowing what is already there is what stops `ow consult add` breaking the configuration it was asked to extend. Over `@iarna/toml` for being ESM, current, and considerably smaller.
