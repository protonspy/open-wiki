...
## 11 · Delta — system prompt and harness entry as user message

- [x] 11.1 (Unit) Add a fixed, application-authored system prompt in `apps/desktop/src/main/agent/system-prompt.ts` (exported `SYSTEM_PROMPT`) that frames what the wiki is and how the agent behaves (product-level framing, not a re-statement of the convention), and pass it as `createAgent`'s `systemPrompt` instead of the harness entry — R2.3
- [x] 11.2 (Unit) Resolve the harness entry as `CLAUDE.md` at the project root when present, otherwise `AGENTS.md`, read with `assertWithin` + real-path (unchanged safety); expose the resolved entry (`{ content, path } | null`) on `EmbeddedAgent` so the chat control can inject it. A project with neither resolves to `null` — R2.3, R2.9
- [x] 11.3 (Unit) In `chat-control.ts`, inject the harness entry as the first user message on an empty thread: `send` checks the thread's checkpoint state, and on the first turn sends `[{ role: "user", content: harnessEntry }, { role: "user", content: text }]`; on later turns sends only the user's text (the checkpointer carries the entry). `resume` is unchanged (a resume is not a user message) — R2.9
- [x] 11.4 (Unit) Test that the fixed `SYSTEM_PROMPT` is what `createAgent` receives as `systemPrompt` and that the harness entry is not; that `resolveHarnessEntry` prefers `CLAUDE.md`, falls back to `AGENTS.md`, and returns `null` when neither is present; and that the first send on a thread prepends the harness entry as a user message while a second send does not — R2.3, R2.9
